<template>
  <div
    class="w-vw h-vh bg-green flex flex-col justify-evenly items-center lg:flex-row"
  >
    ...
    <div
      ref="circle"
      class="w-76 h-76 lg:w-128 lg:h-128 rounded-full bg-pale overflow-hidden flex flex-col justify-center items-center"
    >
      <div ref="svg" :style="backgroundTransform"></div>
      <div
        class="card-mono w-64 lg:w-76 z-10 p-2 flex flex-col justify-center items-center"
      >
    ...
    <div
      class="w-48 h-10 card-mono transition-all duration-150 ease-in-out hover:cursor-pointer transform translate-y-0 hover:translate-y-hov-sm flex flex-row"
    ...
    >
      ...
      <div class="flex-grow flex flex-row justify-center items-center">
        <a
          ...
          target="_blank"
          class="font-sans font-semibold text-sm"
          >VIEW UPDATES</a
        >
      </div>
    </div>
  ...
  methods: {
    setBackgroundTransform(): void {
      this.backgroundTransform = {
        transform: `translate(-${this.w * 0.5}px, -${this.h * 0.5}px)`
      }
    },
    updateGrid(): void {
    ...
    createGrid(pct: number): string {
      ...
      const svg = [
        `<svg width="${this.w * 1.5}px" height="${this.h *
          1.5}px" viewBox="${-offset + 1} ${offset +
          1} 8 8" style="position: absolute">`
      ]

      for (let i = 0; i < stepCount; i++) {
        const pos = i * spacing
        svg.push(
          `<line x1="${pos}" y1="0" x2="${pos}" y2="10" stroke="#98E2C6" stroke-width="1px" vector-effect="non-scaling-stroke" />`
        )
        svg.push(
          `<line x1="0" y1="${pos}" x2="10" y2="${pos}" stroke="#98E2C6" stroke-width="1px" vector-effect="non-scaling-stroke" />`
        )
      }
