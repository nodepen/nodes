...
export const VisibilityButton = ({ nodeInstanceId }: ButtonProps) => {
    const { apply, clearInterface } = useDispatch()

    const isVisible = useStore((state) => state.document.nodes[nodeInstanceId]?.status.isVisible)
    ...
    const label = isVisible ? 'Hide component' : 'Show component'

    return <MenuButton icon={icon} label={label} action={handleToggleVisibility} />
}