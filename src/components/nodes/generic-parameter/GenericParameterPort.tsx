...
type GenericParameterPortProps = {
  nodeInstanceId: string
  portInstanceId: string
  template: NodePen.PortTemplate
}

const GenericParameterPort = ({ nodeInstanceId, portInstanceId, template }: GenericParameterPortProps) => {
  ...
  return (
    <g id={`generic-param-port-${portInstanceId}`} ref={portRef}>
      <circle
        r={NODE_PORT_RADIUS}
        cx={position.x}
        cy={position.y}
...
export default React.memo(GenericParameterPort)