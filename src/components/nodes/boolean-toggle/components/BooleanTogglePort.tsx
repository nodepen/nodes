...
type BooleanTogglePortProps = {
    nodeInstanceId: string
    portInstanceId: string
    template: NodePen.PortTemplate
}

const BooleanTogglePort = ({ nodeInstanceId, portInstanceId, template }: BooleanTogglePortProps) => {
    ...
    return (
        <g id={`boolean-toggle-port-${portInstanceId}`} ref={portRef}>
            <circle
                r={NODE_PORT_RADIUS}
                cx={position.x}
                cy={position.y}
...
export default React.memo(BooleanTogglePort)
