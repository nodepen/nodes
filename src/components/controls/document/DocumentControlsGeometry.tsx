...
export const DocumentControlsGeometry = ({ nodeInstanceId, portInstanceId, valueType }: DocumentControlsGeometryProps) => {
    ...
    return (
        <div className="np-w-full np-flex np-items-center np-justify-between np-gap-1">
            <p className={`np-pl-1 np-pt-1 np-min-w-0 np-truncate np-text-xs np-font-panel np-select-none ${hasValue ? 'np-text-dark' : 'np-text-grey-2 hover:np-text-dark hover:np-cursor-pointer'}`} onClick={hasValue ? () => '' : handlePickGeometry}>
