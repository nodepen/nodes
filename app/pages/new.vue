<template>
  <div class="w-full h-full flex flex-col">
    ...
    <div class="h-12 pl-8 pr-8 bg-green flex flex-row">
      <div
        class="w-full max-w-screen-xs h-full mr-6 flex flex-row items-center"
      >
        <div
          class="div-mono text-sm leading-6 border-l-2 border-t-2 border-r-2 border-solid border-dark h-8 text-center w-1/2"
          @click="toggleCategory"
        >
          {{ selectedCategory }}
        </div>
        <div
          class="div-mono text-sm leading-6 border-t-2 border-r-2 border-solid border-dark h-8 text-center flex-grow w-1/2"
      ...
      <div
        class="h-full pt-3 max-w-3xl overflow-hidden select-none whitespace-no-wrap"
      >
        <div
          v-for="(comp, i) in activeComponents"
          :key="`${i}-${comp.NickName}`"
    ...
    <div
      ...
      :style="{
        position: 'fixed',
        left: `${pipPosition[0] - 10}px`,
        top: `${pipPosition[1] - 10}px`
      }"
...
export default Vue.extend({
  ...
  computed: {
    activeComponents(): Grasshopper.Component[] {
      return (this.components as Grasshopper.Component[])
        .filter(
          (c) =>
            c.Category === this.selectedCategory &&
            c.Subcategory === this.selectedSubcategory
        )
        .sort((a, b) => {
  ...
  methods: {
    async doTest(x: number, y: number, z: number): Promise<void> {
      const res = await this.$axios.$get(
        `http://localhost:8081/test?x=${x}&y=${y}&z=${z}`
      )
    ...
    async getServerConfig() {
      const res: Grasshopper.Component[] = await this.$axios.$get(
        'http://localhost:8081/grasshopper'
      )
      res.forEach((c) => {
    ...
    toggleSubcategory() {
      ...
      const i = cat?.subcategories.findIndex(
        (sub: string) => sub === this.selectedSubcategory
      )
    ...
    handleEndComponentCreate(_e: PointerEvent) {
      if (!this.creating) {
        return
      }
      this.creating = false
    ...
    handleDeselect(): void {
      this.graph.components.forEach((_c, i) => {
        Vue.set(
          this.graph.components,
          i,
          Object.assign(this.graph.components[i], { selected: false })
        )
      })
    }
  }
