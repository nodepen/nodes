...
const { NODE_INTERNAL_PADDING, NODE_MINIMUM_HEIGHT, NODE_PORT_HEIGHT } = DIMENSIONS

export const getNodeHeight = (template: NodePen.NodeTemplate): number => {
  const { inputs, outputs } = template

  const nodeParameterHeight = Math.max(inputs.length, outputs.length, 1) * NODE_PORT_HEIGHT
