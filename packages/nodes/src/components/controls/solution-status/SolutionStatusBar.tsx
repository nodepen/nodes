...
export const SolutionStatusBar = (): React.ReactElement => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setProgress((currentProgress) => {
        const progressStates = [0, 0.25, 0.5, 0.75, 1]

        const currentProgressIndex = progressStates.findIndex((n) => n === currentProgress)

        return progressStates[(currentProgressIndex + 1) % 5]
      })
    }, 2000)
  ...
  return (
    <div className="np-flex-grow np-h-8" ref={shadowTarget} id="wtf">
